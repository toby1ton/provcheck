...
Verify every pinned dependency automatically — on PRs that touch
requirements and on a weekly schedule (upstream tampering can happen
*after* you merge):
...
Results appear in the job summary as a table; findings at or above the
`fail-on` level fail the check. See `examples/workflow.yml` for a
complete workflow. The scanner is `provcheck_ci.py` and runs anywhere,
not just in Actions.
